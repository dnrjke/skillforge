...
## 3. 매핑 테이블 (BattleScene.js)
아래 매핑은 **절대 변경하지 않는다**.
모든 UI 업데이트 로직은 이 테이블을 기준으로 동작해야 한다.

ACTIVE_SLOTS = [1, 2, 4]  // 후열 하단, 중열 상단, 전열 상단

units[0] (후열하, x:150) → pos=1 (뒷줄 중앙, left:98px)
units[1] (중열상, x:260) → pos=2 (뒷줄 상단, left:141px)
units[2] (전열상, x:400) → pos=4 (앞줄 중앙, left:116px)
